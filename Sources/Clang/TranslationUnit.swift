...
public class TranslationUnit {
    ...
    public var diagnostics: [Diagnostic] {
        let diagnosticSet = clang_getDiagnosticSetFromTU(clang)
        ...
        return (0 ..< count).compactMap { idx in
            clang_getDiagnosticInSet(diagnosticSet, idx).map { Diagnostic(clang: $0) }
        }
    }
    ...
    public func getFile(for name: String) -> File? {
        guard let cxfile = clang_getFile(clang, name) else { return nil }
    ...
    public var defaultReparseOptions: TranslationUnitOptions {
        return TranslationUnitOptions(rawValue:
            clang_defaultReparseOptions(clang))
    }
    ...
    public func reparseTransaltionUnit(using unsavedFiles: [UnsavedFile],
        ...
        var cxUnsavedFiles = unsavedFiles.map { $0.clang }
        let err = CXErrorCode(
            UInt32(clang_reparseTranslationUnit(clang,
    ...
    public func saveTranslationUnit(in filename: String,
        ...
        let err = CXSaveError(
            UInt32(clang_saveTranslationUnit(clang, filename, options.rawValue)))
